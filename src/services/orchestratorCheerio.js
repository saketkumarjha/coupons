// src/services/orchestratorCheerio.js
...
async function runScraping() {
  ...
  logger.info(`🚀 Starting scrape run (Cheerio-only): ${runId}`);
  ...
  try {
    ...
    logger.info(
      `✅ ${uniqueCoupons.length} unique coupons (removed ${
        enrichedCoupons.length - uniqueCoupons.length
      } duplicates)`
    );
    ...
    logger.info(
      `✅ ${validCoupons.length} valid coupons (rejected ${
        uniqueCoupons.length - validCoupons.length
      })`
    );
    ...
    logger.info(
      `✅ Saved: ${saveResult.added} new, ${saveResult.updated} updated`
    );
...
async function scrapeAllSources(stores) {
  const scrapers = [
    new CouponDuniaScraperCheerio(),
    new GrabOnScraperCheerio(),
    new CashKaroScraperCheerio(),
  ];
  ...
  results.forEach((result, index) => {
    if (result.status === "fulfilled") {
      logger.info(
        `✅ ${scrapers[index].sourceName}: ${result.value.length} coupons`
      );
      allCoupons.push(...result.value);
    } else {
      logger.error(
        `❌ ${scrapers[index].sourceName} failed:`,
